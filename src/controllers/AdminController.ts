...
> {
  ...
  protected async beforeShow(): Promise<void | NextResponse> {
    ...
    // If route param exists, validate user can only access their own profile
    if(parseInt(user.id) != parseInt(id)){
        return this.sendError("Validation failed", { authentication: "You don't have access to this profile" }, 422);
    }
  }
