...
import AdminPetController from "@/controllers/AdminPetController";
...
export async function GET(
  ...
  try {
    const controller = new AdminPetController(_req);
...
export async function PATCH(
  ...
  try {
    if (contentType.includes("multipart/form-data")) {
      ...
      for (const [key, value] of formData.entries()) {
        if (typeof value === "string") {
          ...
          const uploadDir = path.join(process.cwd(), "public", "uploads", "pet");
          ...
          (data as Record<string, any>).imageUrl = `/uploads/pet/${fileName}`;
        }
      }
    ...
    const controller = new AdminPetController(request, data);
