...
export async function GET(
  ...
  try {
    const controller = new AdminServiceController(_req);
...
export async function PATCH(
  ...
  let data: Partial<ExtendedAdminService> = {};
  ...
  try {
    if (contentType.includes("multipart/form-data")) {
      ...
      for (const [key, value] of formData.entries()) {
        if (typeof value === "string") {
          ...
          (data as Record<string, any>).imageUrl = `/uploads/service/${fileName}`;
        }
      }
    ...
    const controller = new AdminServiceController(request, data);
