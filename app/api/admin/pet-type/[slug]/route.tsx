...
export async function GET(
  ...
  try {
    const controller = new AdminPetTypeController(_req);
...
export async function PATCH(
  ...
  let data: Partial<ExtendedPetType> = {};
  ...
  try {
    if (contentType.includes("multipart/form-data")) {
      ...
      for (const [key, value] of formData.entries()) {
        if (typeof value === "string") {
          ...
          const uploadDir = path.join(process.cwd(), "public", "uploads", "pet-type");
          ...
          (data as Record<string, any>).imageUrl = `/uploads/pet-type/${fileName}`;
        }
      }
    ...
    const controller = new AdminPetTypeController(request, data);
