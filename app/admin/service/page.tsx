...
export default function ServiceList() {
  ...
  const handleDelete = async () => {
    ...
    await fetch(`/api/admin/service/${deleteSlug}`, {
      method: "DELETE",
    });
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
            {loading ? (
              ...
              serviceItems.map((serviceItem) => (
                <TableRow key={serviceItem.id} className="">
                  ...
                  <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <div className="max-w-xs truncate" title={serviceItem.slug || ""}>
                      {serviceItem.slug || "N/A"}
                    </div>
                  </TableCell>
                  <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <Badge
                      ...
                      color={
                        serviceItem.status === true
                          ? "success"
                          : serviceItem.status === false
                          ? "warning"
                          : "error"
                      }
                    >
                      {serviceItem.status === true ? "Active" : "Inactive"}
                    </Badge>
                  </TableCell>
                  <TableCell className="py-3 text-center">
                <ActionMenu
                    editUrl={`/admin/service/edit/${serviceItem.slug}`}
                    onDelete={() => {
                      setDeleteSlug(serviceItem.slug);
